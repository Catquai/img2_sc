...
def clean_spill(r: int, g: int, b: int) -> tuple[int, int, int]:
    # Only remove obvious magenta-key contamination. Do not change normal cyan
    # shield glass, coral handle, or warm shadows.
    magenta_excess = min(r, b) - g
    if magenta_excess > 18:
        if r > 185 and b > 150:
...
def process(
    ...
    for y in range(image.height):
        for x in range(image.width):
            # Key-like pixels are background by default, even when they are trapped
            # between separate foreground elements. A foreground subject that needs a
            # real magenta/purple material must choose a different key color and
            # declare that material in JSON.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Keep key-like pixels that are not connected to the canvas edge. Use only when JSON explicitly declares the key-like color as subject material.",
    )
    parser.add_argument(
        "--alpha-input",
        action="store_true",
