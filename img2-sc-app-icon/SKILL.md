...
description: 使用 Codex 内置图像生成能力，根据文本描述、移动端 App 主图标参考图，或参考图结合文本改写生成 iOS/Android 应用主 icon；默认不要求调用 gpt-image-2 CLI，不依赖 OPENAI_API_KEY。适用于 App logo、launcher icon、store icon、圆角方形主图标、参考图重生成、构图参考与变体重组、风格变体、主体替换、固定尺寸输出、透明 PNG，以及输出透明前景层、完整背景层、白色负形透明图标和本地合成检查图；必须先生成结构化 JSON，保持图标身份、语义层级、风格、画布外形与尺寸约束，但 reference_only 默认必须明显重组构图并降低雷同性。默认流程不直接生成最终合成图，而是先从结构化 JSON 分别生成带自适应纯色键幕背景的前景层和完整背景层；透明前景与白色负形图都必须先有键幕源图，再本地去键色，最后调整尺寸，其中负形图必须先生成带键幕的白色负形源图、本地去键色后将整张画布缩放为 72×72；透明前景主体缩小至 70% 并输出 512×512，同时仅本地合成未缩小主体检查图 `composite_full_subject_preview`；不默认生成 70% 合成预览图 `composite_preview`。
---
...
生成可作为移动端 App 应用主 icon 的 1:1 图像资产。未指定尺寸时，默认交付 `512×512` 透明前景层、`512×512` 完整背景层、`72×72` 白色负形透明图标，以及由未缩小透明前景/背景本地合成得到的 `512×512` 检查图 `composite_full_subject_preview`。主 icon 必须在小尺寸下清晰可辨，拥有一个明确视觉中心，避免像普通 UI 功能图标、插画海报或复杂场景。

始终先判定输入模式并构建结构化 JSON，再调用 Codex 内置图像生成工具；不要要求用户启用或调用 `gpt-image-2` CLI，也不要把 `OPENAI_API_KEY` 作为默认前置条件。默认不要让模型直接生成最终合成图；最终合成图只能由本地透明前景层和背景层合成得到，用作验证预览或交付预览。详细 JSON 字段与检查清单见 `references/app-icon-schema.md`。
...
在构建 JSON 前，必须先给用户提供的每张图片分配一个且仅一个角色，并把判定写入 JSON 的 `input_image_role` / `layer_split` 字段。不要只因为图片看起来像一张完整 icon，就自动当成 `composite_source`。

- `reference_image`：默认角色。用户只提供图片、说“参考这张生成/重新生成/做变体/生成类似图/按这个风格”，或没有明确要求锁定主体、拆这张图、只替换背景时，都判定为参考图。流程必须走 `reference_only` 或 `reference_plus_text`：从图片提炼结构化 JSON，然后生成新的前景变体层和新的完整背景变体层；不得把原图主体直接换成键幕。
- `composite_source`：只有用户明确要求“按这张完成图拆层/从这张图拆前景和背景/锁定这张图/保持主体不变/只替换背景成键幕/删除图中前景补背景/局部重建这张图背景”，或当前上下文中已有一张用户明确接受的生成结果并要求“继续拆分/按刚才那张拆/从新变体图中拆分”时才启用。流程必须走锁定拆分：先从该完成图提取 `foreground_visual_lock`，前景 pass 只能把同一主体换到纯色键幕上，背景 pass 只能删除该主体并补全被遮挡背景。
- `foreground_source_image`：用户提供的图片本身已经是带键幕前景、透明前景、或明确说“这是前景图”。流程不得重新生成前景主体；只能去键色/清理 alpha/缩放到目标尺寸，并基于该前景生成负形图或与背景检查合成。
- `background_source_image`：用户提供的图片本身已经是背景层，或明确说“这是背景图”。流程不得重新生成背景主体内容；只能填充透明区、修复背景、缩放到目标尺寸，并用于后续本地检查。

判定优先级从高到低：显式图层角色词（前景图/背景图/透明前景/绿幕前景） > 显式锁定拆分词（拆层/保持主体/只换背景/删除前景/局部重建这张图） > 已接受生成结果续作（继续拆分/按刚才那张/从新变体图中拆） > 普通参考图。若多个图片同时出现，必须分别标注每张图的角色；不能把参考图、前景图、背景图和已接受合成图混用为同一个来源。
...
2. `reference_only`
   ...
   - 默认任务是生成新的 App icon 图层资产：先提炼参考图 JSON，再按 JSON 分别生成前景层和背景层，最后仅本地生成未缩小主体合成检查图 `composite_full_subject_preview`。不要直接请求模型输出最终合成图，也不要默认生成 70% 合成预览图 `composite_preview`。
   - 提炼 JSON 时必须输出确定语义，不得把候选项或模糊选项写进字段值。禁止使用 `A/B`、`A | B`、`or`、`maybe`、`folder or tray`、`folder/container`、`open folder or tray` 这类让模型自行选择的写法；必须根据参考图视觉证据单选并写成具体对象。例如当前参考图是扁平黄色文件夹时，前景主元素必须写成 `flat_folder`，而不是 `flat_folder_pocket`、`open_folder_tray` 或 `folder_or_container`。
   ...
   - 同一参考图连续重新生成时，不得只在同一构图模板上替换键色、增加一张卡片、微调箭头或移动徽章；但也不得为了变化破坏主元素识别性。连续生成结果至少改变 2 个可见维度：主体组织方式、主箭头位置/路径、卡片数量与排列、前后遮挡关系、主重心区域、主体/点缀配色、背景元素家族、背景配色或背景构图。对文件夹上传类图标，必须基于参考图视觉证据把主容器单选为 `flat_folder` 或 `deep_tray_box`，并把该确定值写入 JSON。参考图若是扁平黄色文件夹/文件袋，只能提炼为 `flat_folder`：保留文件夹前片、后片、斜向开口、右侧折片/标签和浅口袋关系；不得写成 deep tray、storage bin、box frame、basket、side-wall container 或 thick rectangular frame。只有参考图本身明确是深托盘/盒子时，才允许 `deep_tray_box`、侧壁和内腔等词。若改变主体外轮廓会降低识别性，必须改用构图、配色、箭头路径、卡片排列和背景变化来拉开差异。
...
默认将 `output.mode` 设为 `foreground_background_pair`，并在生成前建立 `layer_split`。不要直接让模型生成最终合成图；最终合成图只能由本地脚本把透明前景层叠加到背景层得到。只有用户明确要求“只要单张图/不要拆层”时，才切换为 `composite`。用户只要求“不生成负形图”时，不改变双层输出模式。
...
严禁用本地粗略 mask、手绘多边形遮罩、阈值/连通域抠图、简单抠原参考图、局部旋转/缩放参考图等方式从完整合成图直接创造前景层、背景层或负形图。如果图像生成工具不可用，必须停止并说明无法生成，不得交付本地粗处理 fallback。

**Hard stop：本地透明处理只能处理已有透明/键幕中间图，不能创造图层。**

在运行任何本地 mask、alpha、去幕、透明化、裁切或缩放脚本前，必须先确认输入来源：
- 若输入是完整合成图、普通参考图或 `composite_source` 原图，本地脚本只能读取尺寸、分析颜色、验证结果；不得输出前景层、背景层、负形图或透明 PNG 交付物。
- 若需要新建透明前景、透明负形孔洞、透明角或透明背景，必须先由图像生成工具生成纯色键幕中间图，再本地去键幕得到真实 alpha PNG。
- 若输入已经是用户明确提供的 `foreground_source_image`，且它本身带真实 alpha 或纯色键幕，允许只做 alpha 清理、去键、缩放、负形派生和合成检查；不得重抠完整合成图。
- 若输入已经是用户明确提供的 `background_source_image`，只允许填充透明区、修复/缩放为不透明背景；不得从完整合成图本地抠掉主体来冒充背景生成。
- 负形图不得从原始完成图或粗抠 alpha 直接填白生成；必须基于实际 `foreground_keyed_source` 或用户提供的真实前景层建立负形结构，再通过键幕/alpha 规范化得到纯白透明图标。

允许的本地处理仅限：去除模型生成或用户提供的键幕、清理用户提供的真实 alpha、缩放、纯白规范化、背景不透明化、合成检查、尺寸/alpha/纯白像素验证。除此之外，只要本地处理在“创造图层内容”，就必须停止并改为生成键幕中间图或报告阻塞。
...
默认输出必须产出六类文件：带自适应纯色键幕主体层、未缩放透明前景图、70% 透明前景图、完整背景图、白色负形透明图标、未缩小主体合成检查图 `composite_full_subject_preview`。若用户关闭负形图，则产出五类文件并省略白色负形透明图标。不默认生成 70% 前景合成预览图 `composite_preview`。
...
3. 根据 JSON 背景元素生成完整背景层 `background_png`，并缩放/规范到 `512×512`。若是 `reference_only` 新变体，背景 pass 必须生成新的完整变体背景层；若存在已接受的 `composite_source` 并要求拆分该完成图，背景 pass 才执行“删除前景元素并补全原背景”，且不允许改变未被前景遮挡的背景元素。两种情况都不得包含主体残影、主体形状空洞或重复前景装饰。
4. 在原始尺寸或更高尺寸下本地去键色，清理键色污染，输出透明前景层 `foreground_alpha_png`。不得先缩放 keyed source 再去幕。
5. 若 `white_negative_icon.enabled` 为 `true`，根据当前实际带键幕前景层 `foreground_keyed_source` 生成带纯色键幕背景的白色负形源图 `white_negative_keyed_source`，保留可识别的内部负形结构；随后必须本地去键色得到原始尺寸透明负形图，再将整张图像画布整体缩放到 `72×72` 输出 `white_negative_icon_png`。负形图不做主体 70% 缩小，不做 trim 后重新居中；若用户关闭负形图，跳过此步骤并在输出清单中说明。
...
8. 默认到此结束，不再生成 70% 前景合成预览图 `composite_preview`。只有用户明确要求额外查看 70% 合成效果时，才允许用 `512×512` 透明前景图与 `512×512` 背景图本地合成临时预览；该临时预览不得列为默认交付物。

### 5. 默认极简白色负形透明图标

默认将 `white_negative_icon.enabled` 设为 `true`，仅在用户明确要求不要负形图时关闭。必须根据当前任务实际生成的带键幕前景层 `foreground_keyed_source` 生成极简白色负形图标：保留主要元素轮廓和必要内部负形结构，使用纯白主体与透明负形孔洞表达身份特征。负形图透明输出必须强制走三段式流程：先生成带纯色键幕背景的 `white_negative_keyed_source`，再本地去键幕得到原始尺寸透明负形图，最后将整张图像画布缩放到 `72×72`；不得直接从语义描述、历史 alpha、前景 alpha 填白或模型透明输出跳到最终 `72×72`。

目标视觉规则：

- 整体为极简单色图标，所有可见像素只能是纯白色 `#ffffff`；纯白色单色填充，无任何其他颜色。
- 最终交付背景必须真实透明；但在此之前必须先存在带纯色键幕背景的白色负形源图，透明 alpha 只能由本地去键幕得到。
- 细节必须采用镂空/透明负形处理，不允许使用描边、阴影、渐变、灰色、彩色、发光、纹理、半透明填充、3D 材质或明暗层次表达细节。
- 层级只能依赖白色填充形状与透明负形孔洞之间的形状对比表达；不得通过线框、描边、色阶、透明度变化或局部高光来表达层级。
- 外轮廓保持主体最重要的识别特征，但允许简化细碎结构。
- 负形图必须基于当前 `foreground_keyed_source` 的实际主体轮廓、部件位置、遮挡关系和内部特征生成；不得从 `foreground_alpha_png`、语义描述、历史对话或通用模板重新设计一张“相似概念”的白色图标。对于文件夹上传类图标，必须按 `foreground_keyed_source` 中已解析的容器类型锁定外轮廓、开口结构、卡片数量与角度、主上传箭头路径与位置、正面徽章位置与内部上传标记；不能把中央主箭头删掉、把卡片改角度、把 flat_folder 改成托盘/普通盒子，或只保留徽章里的小箭头。
...
2. 只能从 `foreground_keyed_source` 建立 `negative_visual_lock`：记录主体外轮廓、每个前景部件的 bbox、旋转角度、遮挡顺序、主箭头/工具/徽章等身份部件的位置，以及哪些彩色内部图形要转为透明负形孔洞。`foreground_alpha_png` 只能作为校验或去幕结果，不得作为负形图语义/构图来源。
3. 创建负形设计 JSON，明确 `white_fill_regions`、`transparent_negative_regions`、最小负形宽度和禁止删除的身份特征。`white_fill_regions` 必须来自 `foreground_keyed_source` 中的真实前景部件，`transparent_negative_regions` 必须来自 `foreground_keyed_source` 中的实际内部图形、开口或部件分隔，不得凭空新增模板化孔洞。
4. 使用图像生成工具或本地结构化图像处理基于 `foreground_keyed_source` 生成“纯白主体 + 键幕负形孔洞/键幕背景”的 `white_negative_keyed_source`。若使用模型生成，必须把当前 `foreground_keyed_source` 图像文件作为输入/参考图传入，并在提示词中明确 `match the supplied foreground silhouette and part positions exactly, do not redesign the icon composition`、`pure white visible pixels only on a perfectly uniform selected chroma-key background`、`negative-space details must use the chroma-key color`。禁止只用语义描述、历史对话、上一轮负形图或通用模板生成负形图。若当前图像工具不能附带该前景图文件，必须改用本地 alpha/vector 处理构造同样的键幕负形源图，或停止并说明负形图阻塞。
5. 对 `white_negative_keyed_source` 本地去键幕，输出原始尺寸透明负形图 `<white-negative-alpha-original-size>`。只有键幕色区域可以变成透明；所有可见像素必须保留为主体候选，不得依赖模型直接输出 alpha，也不得跳过键幕源图直接从前景 alpha 填白交付。
6. 使用 `normalize_white_alpha_icon.ps1` 将原始尺寸透明负形图的所有可见像素强制规范为纯白并保留 alpha。
7. 将规范化后的整张负形图像画布缩放到 `72×72`，不裁切主体后再缩小，不添加额外安全边距，不使用 `-ScalePercent`。
8. 使用 `check_white_alpha_icon.ps1` 验证只有纯白可见像素和透明像素，并验证背景真实透明。
...
禁止把前景 alpha 直接填充成白色后当作负形图标交付，也禁止直接本地从前景 alpha 生成最终透明负形图而没有 `white_negative_keyed_source`。白色负形图必须先形成带键幕背景和键幕负形孔洞的源图，再本地去幕。白色负形图必须保留足够的内部透明负形结构，例如盾牌内外层分隔、扫帚刷头/手柄分隔、垃圾桶桶身槽线等决定身份的结构；如果 72×72 下变成白色土豆块或无法识别，判定失败。
同样禁止让模型自由重绘负形构图后交付。若负形 keyed source 与前景 keyed source 的主体轮廓、主要部件数量、卡片/工具角度、主箭头位置、徽章位置或关键遮挡关系不一致，即使最终是纯白透明 PNG，也判定失败，必须回到 `foreground_keyed_source` 重新提取实际轮廓和内部特征。

规范化与验证必须覆盖完整链路，不能只运行颜色规范化：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/remove_green_screen.ps1 -Source "<white-negative-keyed-source>" -Output "<white-negative-alpha-original-size>" -KeyColor "<selected-key-color>"
...
- 文件夹/上传类图标的主容器是核心识别对象：必须按参考图单选 `flat_folder` 或 `deep_tray_box`，不得把二者并列写入 JSON 或提示词。多数文件夹上传参考图是扁平文件夹，而不是盒子；这类 JSON 应使用确定短语 `flat yellow file folder with front panel, rear flap, angled open mouth, shallow pocket relationship, and right folded side flap`，并明确禁止 `box frame`、`deep tray`、`storage bin`、`basket`、`side-wall container`、`thick rectangular rim`、`generic paper box`。只有参考图真的有明显高侧壁和深内腔时，才使用 `deep_tray_box`。可以改变视角、开口朝向、卡片分布、箭头路径、局部配色和背景风格，但不得把扁平文件夹变成框、盒、细长竖盒、抽象柱体或无法看出 flat_folder 的形状。
...
默认调用 Codex 内置图像生成工具完成各生成 pass，不要求调用 `gpt-image-2` CLI，不要求安装额外 CLI，也不要求用户提供 `OPENAI_API_KEY`。将最终 JSON 作为主要提示词依据，并强调：
...
默认 `foreground_background_pair` 流程中禁止使用“生成单张完整 icon / composite / final app icon / full opaque icon”的提示词。除非用户明确要求 `composite` 单图输出，否则每次图像生成调用都必须声明本 pass 的目标是 `foreground_keyed_source`、`background_png` 或 `white_negative_icon_png` 之一；默认只允许本地生成未缩小主体合成检查图 `composite_full_subject_preview`，不生成 70% 前景合成预览图 `composite_preview`。

```text
Generate one layer asset for a mobile App primary icon from this structured JSON.
This is not a final composite icon request. Produce only the requested layer pass:
foreground_keyed_source, background_png, or white_negative_icon_png.
Preserve every identity, required trait, relationship, invariant, canvas rule,
foreground/background ownership rule, and failure criterion.
Do not create the final composite preview, UI screens, icon grids, device mockups,
or explanatory text.
```

在 `reference_only` 模式中追加：先在本地完成参考图语义解析，把容器类型解析成一个确定值，再把该确定值写入 JSON 和生图提示；不要把分类过程或候选词写进生图提示。例如当前参考图应写 `primary container type: flat_folder; primary container: flat yellow file folder with front panel, rear flap, angled open mouth, shallow pocket relationship, and right folded side flap`，不写 `folder or tray`。随后追加：`Create a visibly new App icon variant, not a cleaned-up copy. Preserve semantic identity, style family, required parts, hierarchy, material readability, contrast, safe-area fit, and small-size recognition, but do not preserve the reference composition by default. Before writing the image prompt, copy resolved_primary_container.type, prompt_phrase, required_traits, and forbidden_traits verbatim from JSON into the prompt. Do not use synonyms that broaden flat_folder into tray, container, box, basket, side-wall container, or deep cavity. Randomly sample a fresh set of variation parameters for this generation; do not reuse the reference pose or any previous default template. If this is a retry or repeated generation for the same reference, also avoid the previous generated composition family, not only the original reference. Change at least three of these dimensions in a visible way: silhouette/local shape where it does not harm recognition, pose/view angle, rotation, scale, center of mass, relative element positions, overlap order, subject colorway, accent colors, background element family, background palette, background layout, and decorative layout. For repeated generations, change at least two visible dimensions such as subject organization, arrow path/placement, card count and arrangement, overlap order, main center of mass, subject/accent colorway, background element family, background palette, or background composition family. Do not achieve variation by deforming the primary object until it is no longer recognizable. Preserve the resolved primary container semantics exactly. If the resolved container type is flat_folder, do not render it as a box frame, deep tray, storage bin, basket, side-wall container, thick rectangular rim, thin vertical box, abstract pillar, generic paper box, or any object with visible high side walls/deep inner cavity. If the subject is a single primary element, introduce clear but readable shape, pose, random direction-angle rotation, scale, center-shift, and color variation. If the subject has multiple elements, preserve the main element identity while visibly reorganizing element positions, direction angles, scale relationships, overlap, spacing, and colors. Randomly vary decorative accents such as sparkles, light dots, particles, badges, small highlights, background glints, and shine fragments by changing their count, position, scale, angle, opacity, brightness, color, or shape details while keeping them secondary. Reject a result that only makes minor color/highlight changes or keeps the same silhouette, pose, overlap, and element placement as the reference or the previous generation. Reject a result whose variation comes mainly from identity-breaking deformation of the resolved primary container. Reject a flat_folder reference result if the folder becomes a box/frame/deep tray. For composite output, fill the entire 1:1 canvas with an opaque background unless transparent PNG is explicitly requested. Preserve semantic orientation and required parts.`
...
Resolved container enforcement for the foreground pass: before writing the image prompt, copy `resolved_primary_container.type`, `prompt_phrase`, `required_traits`, and `forbidden_traits` from JSON verbatim. If `type` is `flat_folder`, render a flat file folder, not a tray/container/box. The prompt must not contain tray, basket, storage bin, side-wall container, thick rim, visible high side walls, or deep inner cavity unless `type` is explicitly `deep_tray_box`.

Background pass when no composite_source exists: extract the background description from the structured JSON and render a new complete fully opaque background variant. Preserve the background style family, lighting logic, palette family, and decorative grammar, but visibly change at least three independent background dimensions: radial glow position, gradient center, dotted-wave/texture path, particle distribution, light-band direction, local hue balance, or depth rhythm. Do not render the primary subject, its silhouette, ghost, cutout hole, or duplicated foreground decorations.

Background pass when composite_source exists: use the accepted composite_source as the background repair target. Remove the locked foreground subject area and locally/inpaint reconstruct the hidden background so the final layer is complete and fully opaque. Preserve unoccluded background layout, palette, lighting and decoration; do not generate a new background variant, do not leave subject-shaped holes, and do not include foreground ghosts or duplicated foreground decorations.

White negative icon pass: use the actual foreground_keyed_source image file as the visual reference input and render a minimal pure-white negative-space icon on a perfectly uniform selected chroma-key background. Do not rely on conversation memory, semantic description, previous generated icons, or a reused folder/upload template. Preserve the supplied foreground image's subject silhouette, card count, card angles, arrow position, overlap order, and essential internal negative-space identity features. If the image generation tool cannot attach the actual foreground_keyed_source as an input image, do not call the model for this pass; use local alpha/vector processing or stop and report that the negative icon is blocked. Do not render shadows, gradients, gray pixels, texture, glow, or 3D material. Locally remove the key color, then resize the whole final canvas to 72x72; do not apply the 70 percent foreground scaling rule to the negative icon.
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/compose_layer_pair.ps1 -Foreground "<alpha-original-size-or-unscaled-512>" -Background "<background-512>" -Output "<composite-full-subject-preview>"
```

默认不要对 70% 前景层再执行 `compose_layer_pair.ps1` 生成 `composite_preview`。仅当用户明确要求额外预览 70% 合成效果时，才临时生成。
...
- 对 `text_only` 连续生成或用户指出“变化太小”的请求，若结果仍沿用上一轮的主体视角、文件数量/排列、箭头路径/位置、重心区域和背景构图，只是轻微换色、改变高光或重绘材质，判定失败。必须重新抽取并执行新的 `composition_variation.randomization`。
- 对 `output.mode: composite` 的单张合成图，若提示词中没有显式写入本次随机变化参数，或仍复用上一轮相同 bbox/位置关系/背景描述，判定为规则执行失败。
...
- 背景必须证明来自结构化 JSON 的背景信息。`reference_only` 新变体必须生成新的完整变体背景层；`composite_source` 拆分模式必须从完成图删除前景并补全被遮挡区域，不得擅自生成新背景变体。最终背景尺寸为 `512×512`，不得包含透明或半透明像素、主体残影、主体形状空洞或重复前景装饰。
...
- 极简白色负形图标必须基于当前实际 `foreground_keyed_source` 图像文件生成，不能基于语义描述、历史对话、上一轮负形图或通用模板生成。若使用模型生成负形图，必须把实际前景图文件作为输入/参考图传入；若工具不能附带该图，则必须改用本地 alpha/vector 处理或报告阻塞。负形图必须先生成或构造 `white_negative_keyed_source`，再本地去幕变成透明背景，然后将整张图像画布缩放到 `72×72`；只包含纯白可见像素和真实透明像素；无描边、阴影、渐变、灰色或其他颜色；负形孔洞清晰且主体身份可识别。
...
- `scripts/compose_layer_pair.ps1`：验证前景/背景层尺寸与透明度，并生成未缩小主体合成检查图 `composite_full_subject_preview`；默认不得生成 70% 合成预览图 `composite_preview`。
