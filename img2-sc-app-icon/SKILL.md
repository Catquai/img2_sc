...
2. `reference_only`
   ...
   - 提炼 JSON 时必须输出确定语义，不得把候选项或模糊选项写进字段值。禁止使用 `A/B`、`A | B`、`or`、`maybe`、`folder or tray`、`folder/container`、`open folder or tray` 这类让模型自行选择的写法；必须根据参考图视觉证据单选并写成具体对象。例如当前参考图是扁平黄色文件夹口袋时，前景主元素必须写成 `flat_yellow_file_folder_pocket`，而不是 `open_folder_tray` 或 `folder_or_container`。
   ...
   - 同一参考图连续重新生成时，不得只在同一构图模板上替换键色、增加一张卡片、微调箭头或移动徽章；但也不得为了变化破坏主元素识别性。连续生成结果至少改变 2 个可见维度：主体组织方式、主箭头位置/路径、卡片数量与排列、前后遮挡关系、主重心区域、主体/点缀配色、背景元素家族、背景配色或背景构图。对文件夹上传类图标，必须基于参考图视觉证据把主容器单选为 `flat_folder_pocket` 或 `deep_tray_box`，并把该确定值写入 JSON。参考图若是扁平黄色文件夹/文件袋，只能提炼为 `flat_folder_pocket`：保留文件夹前片、后片、斜向开口、右侧折片/标签和浅口袋关系；不得写成 deep tray、storage bin、box frame、basket、side-wall container 或 thick rectangular frame。只有参考图本身明确是深托盘/盒子时，才允许 `deep_tray_box`、侧壁和内腔等词。若改变主体外轮廓会降低识别性，必须改用构图、配色、箭头路径、卡片排列和背景变化来拉开差异。
...
- 负形图必须基于当前 `foreground_keyed_source` 的实际主体轮廓、部件位置、遮挡关系和内部特征生成；不得仅根据语义重新设计一张“相似概念”的白色图标。对于文件夹/托盘上传类图标，文件夹或托盘外轮廓、开口/内腔、卡片数量与角度、主上传箭头路径与位置、正面徽章位置与内部上传标记都属于必须从前景图锁定的特征；不能把中央主箭头删掉、把卡片改角度、把箱体改成普通盒子，或只保留徽章里的小箭头。
...
2. 从 `foreground_keyed_source` 或去幕后的 `foreground_alpha_png` 建立 `negative_visual_lock`：记录主体 alpha 外轮廓、每个前景部件的 bbox、旋转角度、遮挡顺序、主箭头/工具/徽章等身份部件的位置，以及哪些彩色内部图形要转为透明负形孔洞。
3. 创建负形设计 JSON，明确 `white_fill_regions`、`transparent_negative_regions`、最小负形宽度和禁止删除的身份特征。`white_fill_regions` 必须来自真实前景部件，`transparent_negative_regions` 必须来自前景中的实际内部图形、开口、内腔或部件分隔，不得凭空新增模板化孔洞。
...
- 文件夹/上传类图标的文件夹或托盘是核心识别对象：必须按参考图单选 `flat_folder_pocket` 或 `deep_tray_box`，不得把二者并列写入 JSON 或提示词。多数文件夹上传参考图是扁平文件夹口袋，而不是盒子；这类 JSON 应使用确定短语 `flat yellow file folder pocket with front panel, rear flap, angled open mouth, shallow pocket, and right folded side flap`，并明确禁止 `box frame`、`deep tray`、`storage bin`、`basket`、`side-wall container`、`thick rectangular rim`、`generic paper box`。只有参考图真的有明显高侧壁和深内腔时，才使用 `tray/container/side walls/inner cavity`。可以改变视角、开口朝向、卡片分布、箭头路径、局部配色和背景风格，但不得把扁平文件夹变成框、盒、细长竖盒、抽象柱体或无法看出文件夹口袋的形状。
